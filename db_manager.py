...
class DBManager:
    ...
    def get_all_vacancies(self):
        ...
        with self.connection.cursor() as cur:
            cur.execute("""
                    SELECT companies.company_name, vacancies.title, vacancies.salary_from, vacancies.salary_to, vacancies.vacancy_url
                    FROM vacancies
    ...
    def get_vacancies_with_higher_salary(self):
        ...
        with self.connection.cursor() as cur:
            cur.execute("""
                    ...
                    FROM vacancies
                    JOIN companies ON vacancies.company_id = companies.company_id
                    WHERE ((COALESCE(salary_from, 0) + COALESCE(salary_to, 0)) / CASE 
    ...
    def get_vacancies_with_keyword(self, keyword):
        ...
        with self.connection.cursor() as cur:
            # Используем форматирование строки для ILIKE
            ...
            cur.execute(f"""
                ...
                FROM vacancies
                JOIN companies ON vacancies.company_id = companies.company_id
                WHERE vacancies.title ILIKE '{search_pattern}';
            """)
